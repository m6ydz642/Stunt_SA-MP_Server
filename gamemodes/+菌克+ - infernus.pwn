...
#define MAX_GET_CAR 7 // 차량구매 가능한수
...
SetGameModeText("Server Version 0.7");
...
//	SetTimer("TimeUpdate",50000, 1);
	SetTimer("CJCheckpoint", 1, 1); // 체크 포인트
...
// server commands
Textdraw1 = TextDrawCreate(340.000000,410.000000,"command : /help /cj /play2 /aps /stone");
// 410 높이
// 340 왼쪽 오른쪽
...
{
SendClientMessage(playerid,COLOR_RED," + 자살하였습니다 돈 500만원을 가져갑니다");
...
SendClientMessage(playerid, COLOR_YELLOW, " + play4 로 이동 되었습니다 ");
    if(IsPlayerInAnyVehicle(playerid) == 1)
    {
 ...
 if (strcmp(cmdtext, "/nitro", true) == 0)
	{
	    ...
	    if(quest[playerid] == 1) {
		SendClientMessage(playerid,COLOR_YELLOW," + 퀘스트를 완료했습니다. 다음 퀘스트가 기다리고 있으니 /quest 를 쳐보세요");
		SendClientMessage(playerid,COLOR_YELLOW," + 보상 : $200000");
		quest[playerid] = 2;
		//exp[playerid] += 3;
		//GetPlayerLevel(playerid);
		//GiveMoney(playerid, 2000);
	...
	if(IsPlayerAdmin(playerid))
		 {
            ...
            if(IsPlayerConnected(banid))
			{
				...
				if(Waring[banid] == 1)
				{
			  	SendClientMessage(banid, COLOR_RED, " + 경고[1/3] 입니다!");
			  	SendClientMessage(banid, COLOR_RED, " + 경고[1/3] 입니다!");
			  	SendClientMessage(banid, COLOR_RED, " + 경고[1/3] 입니다!");
			  	SendClientMessage(banid, COLOR_RED, " + 경고[1/3] 입니다!");
			  	SendClientMessage(banid, COLOR_RED, " + 경고[1/3] 입니다!");
			  	...
			  	if(Waring[banid] == 2)
				  {
  	SendClientMessage(banid, COLOR_RED, " + 경고[2/3] 입니다 한번만더 경고되면 킥 됩니다");
  	SendClientMessage(banid, COLOR_RED, " + 경고[2/3] 입니다 한번만더 경고되면 킥 됩니다");
  	SendClientMessage(banid, COLOR_RED, " + 경고[2/3] 입니다 한번만더 경고되면 킥 됩니다");
  	SendClientMessage(banid, COLOR_RED, " + 경고[2/3] 입니다 한번만더 경고되면 킥 됩니다");
  	SendClientMessage(banid, COLOR_RED, " + 경고[2/3] 입니다 한번만더 경고되면 킥 됩니다");
  	...
  	SendClientMessage(banid, COLOR_RED, " + 경고[2/3] 입니다 한번만더 경고되면 킥 됩니다");
		...
		//	JailTime[playerid] = 120;
			  	...
			  	if(Waring[banid] == 3)
				  {
			  	SendClientMessage(banid, COLOR_RED, " + 경고[3/3] 입니다! 벌로 킥");
			  	SendClientMessage(banid, COLOR_RED, " + 경고[3/3] 입니다! 벌로 킥");
			  	SendClientMessage(banid, COLOR_RED, " + 경고[3/3] 입니다! 벌로 킥");
			  	SendClientMessage(banid, COLOR_RED, " + 경고[3/3] 입니다! 벌로 킥");
			  	SendClientMessage(banid, COLOR_RED, " + 경고[3/3] 입니다! 벌로 킥");
...
if (GetVehicleModel(vehicleid) == 519 && GetVehicleModel(vehicleid) == 563 && GetVehicleModel(vehicleid) == 487 && GetVehicleModel(vehicleid) == 592 && GetVehicleModel(vehicleid) == 513 )// 샤멜을 탈 경우
	{
...
public chatkicks()
{
	...
	format(string, sizeof(string), " + %d번 %s 님은 채팅창 도배 해서 킥 됬 습니다 ",i,playername);
	SendClientMessageToAll(COLOR_RED, string);
	Kick(i);
	printf(string);
    }
...
public say() //실시간 공지
{
   ...
   switch(saying)
   {
       ...
       case 19:{ SendClientMessageToAll( COLOR_SPRINGGREEN, " + Admin : 차량 가격은 500만원이오 허허허"); }
        case 20:{ SendClientMessageToAll( COLOR_ORANGE, " + 보험 관련 도움말은 /보험도움말 입니다 죽고나면 보험 자동 탈퇴 됩니다"); }
...
udb_UserSetFloat(loginid[playerid],"Ammor",Ammor);
...
{
{
 for(new i=0; i<MAX_PLAYERS; i++) // i 는 현재 접속한 모든 플레이어
 {
SendClientMessage(i, COLOR_RED, " + 현재 당신은 서버 이용 시간이 3시간 경과 되었습니다");
SendClientMessage(i, COLOR_RED, " + 현재 당신은 서버 이용 시간이 3시간 경과 되었습니다");
SendClientMessage(i, COLOR_RED, " + 현재 당신은 서버 이용 시간이 3시간 경과 되었습니다");
SendClientMessage(i, COLOR_RED, " + 현재 당신은 서버 이용 시간이 3시간 경과 되었습니다");
SendClientMessage(i, COLOR_RED, " + 현재 당신은 서버 이용 시간이 3시간 경과 되었습니다");
SendClientMessage(i, COLOR_RED, " + 서버 이용중 감사 하다는 의미로 100만원씩 드리겠습니다");
...
SendClientMessage(playerid,orange," + 낚시로 번돈 15000원이 생겼습니다.");
