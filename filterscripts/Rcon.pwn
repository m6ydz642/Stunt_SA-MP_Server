...
public OnPlayerCommandText(playerid, cmdtext[])
{
	...
	// 명령어 추적을 자동시작 한부분을 주석 처리하여 사용못하게 설정 하엿다
    ...
    dcmd_auth("givefuckingcash",18,givefuckingcash,AUTH_CMD_CASH);
	dcmd_auth("돈배분",6,givefuckingcash,AUTH_CMD_CASH);
	...
	dcmd_auth("정보",4,status,AUTH_CMD_USERINFO);
	dcmd_auth("inforip",4,status,AUTH_CMD_USERINFO);
	...
	dcmd_auth("돈세팅",6,setcashmotherfucker,AUTH_CMD_CASH);
	dcmd_auth("setcashmotherfucker",19,setcashmotherfucker,AUTH_CMD_CASH);
	...
	dcmd_auth("뉘신세요님은",1,find,AUTH_CMD_USERINFO);
	dcmd_auth("user정보보기",1,find,AUTH_CMD_USERINFO);
	...
	dcmd_auth("ping정리하고싶음",1,pingcheck,AUTH_CMD_PINGCHECK);
	dcmd_auth("pingcheck하고싶음",91,pingcheck,AUTH_CMD_PINGCHECK);
	dcmd_auth("핑제한하고싶음",1,pinglimit,AUTH_CMD_PINGCHECK);
	dcmd_auth("setplimit하고싶음",1,pinglimit,AUTH_CMD_PINGCHECK);
	dcmd_auth("핑경고하고싶음",1,pingwarntime,AUTH_CMD_PINGCHECK);
	dcmd_auth("setpwarn울랄라",1,pingwarntime,AUTH_CMD_PINGCHECK);
	dcmd_auth("내리기ㅇㅅㅇㅅㅇㅅㅇㅅㅇ",1,drop,AUTH_CMD_SPECTATE);
...
dcmd_givefuckingcash(playerid,tmp[])
{
...
//==========================================================
dcmd_setcashmotherfucker(playerid,tmp[])
{
...
IsCmdNeedToHide(cmd[])
{
	static hidecmds[][]= // 표시 되게 하지않게 하는 명령어
	{
		//"/부운로그인",
	//	"/sublogin",
		"/log",
